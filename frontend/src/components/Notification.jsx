...
const ShowNotification = () => {
  ...
  return (
    <div className="absolute rounded-md z-10 scrollbar-w-1 scrollbar scrollbar-thumb-primary scrollbar-track-slate-300 overflow-y-auto flex flex-col top-15 left-0 w-72 max-h-72 bg-card shadow-sm text-text transition-all duration-300 ease-in-out">
      {dummyNotif.map((notif) => (
        <div
          key={notif.id}
          className="group flex px-4 py-2 items-start border-b-2 border-foreground last:border-b-0 hover:bg-primary gap-2 cursor-pointer transition-colors duration-200"
        >
          <div>
          ...
          <div>
            <p className="text-xs text-text/50 group-hover:text-text-accent/50">
              {notif.id}
            </p>
